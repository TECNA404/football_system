...
function StandingsPage() {
  ...
  return (
    ...
    >
        ...
        <div className="card-body p-4">
          <div className="row align-items-center">
            ...
            <div className="col">
              ...
              <select
                className="form-select border-0 bg-light bg-opacity-50 fw-bold rounded-3 py-2"
      ...
      {hasTie && (
        <div className="alert alert-warning py-3 px-4 small border-0 shadow-premium mb-4 rounded-4 d-flex align-items-center gap-3">
          <Activity size={18} />
          <span className="fw-bold">{i18n.language === 'uk' ? 'Увага: Деякі команди мають однакові показники' : 'Warning: Some teams have identical stats'}</span>
        </div>
      )}
                  ...
                  {standings.map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td>
                          <Link to={`/teams/${row.team}`} className="text-decoration-none d-flex align-items-center group">
                            <div className="standing-logo-container me-3 shadow-sm rounded-circle overflow-hidden bg-white p-1">
                              {row.team_logo ? (
                            ...
                            <span className="fw-black text-dark group-hover-primary transition-all fs-5">{row.team_name}</span>
                          </Link>
                        </td>
                        ...
                        <td className="text-center">
                          <span className={`badge rounded-pill px-3 ${row.goal_difference > 0 ? "bg-success" : row.goal_difference < 0 ? "bg-danger" : "bg-secondary"} bg-opacity-10 text-${row.goal_difference > 0 ? "success" : row.goal_difference < 0 ? "danger" : "secondary"}`}>
                        ...
                        <td className="pe-4">
                          {note ? (
                            <span className="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 rounded-pill px-3 py-2 small d-flex align-items-center gap-1 w-fit">
                              <Medal size={14} /> {note}
                            </span>
                          ) : isTied ? (
                            <span className="badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25 rounded-pill px-3 py-2 small d-flex align-items-center gap-1 w-fit">
          ...
          >
            <div className="row g-4">
              ...
              <div className="col-md-6 border-start-md">
                <h6 className="fw-black text-uppercase ls-1 text-primary mb-3 d-flex align-items-center gap-2">
                  <Award size={18} /> {i18n.language === 'uk' ? 'Правила тай-брейку' : 'Tie-break Rules'}
                </h6>
